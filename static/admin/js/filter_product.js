document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', async function(event) {
        ...
        if (addRowLink && addRowLink.textContent.includes("Add another Состав изделия")) {
            console.log('Клик на "Add another Состав изделия"');
...
function applyFolderFilterForAllFields() {
    const folderSelectTechnology = document.querySelectorAll('[id^="id_productcomposition_set-"][id$="-folder_technology"]');
    ...
    folderSelectTechnology.forEach((folderSelectTechnology_in, index) => {
        const technologyField = document.querySelector(`#id_productcomposition_set-${index}-technology`);
        const techOperationField = document.querySelector(`#id_productcomposition_set-${index}-techoperation`);
        const folderField = document.querySelector(`#id_productcomposition_set-${index}-folder_technology`);
        const nomenclatureField = document.querySelector(`#id_productcomposition_set-${index}-nomenclature`);
...
function updateTechOperationOptions(technologyId, techOperationField, index) {
    ...
    fetch(`/api/get_technology_of_product/?technology_id=${technologyId}`)
        .then(response => response.json())
        .then(data => {
...
function copyFolderValue() {
    // Находим все поля папок, соответствующие составу изделия
    const folderFields = document.querySelectorAll('[id^="id_productcomposition_set-"][id$="-folder_technology"]');
    ...
    if (folderFields.length > 1) {
        ...
        if (visibleFolderFields.length > 1) {
            ...
            const lastFields = {
                folder: document.querySelector(`#id_productcomposition_set-${lastIndex}-folder_technology`),
                technology: document.querySelector(`#id_productcomposition_set-${lastIndex}-technology`),
                techOperation: document.querySelector(`#id_productcomposition_set-${lastIndex}-techoperation`),
                nomenclature: document.querySelector(`#id_productcomposition_set-${lastIndex}-nomenclature`),
            };
            
            const newFields = {
                folder: document.querySelector(`#id_productcomposition_set-${newIndex}-folder_technology`),
                technology: document.querySelector(`#id_productcomposition_set-${newIndex}-technology`),
                techOperation: document.querySelector(`#id_productcomposition_set-${newIndex}-techoperation`),
                nomenclature: document.querySelector(`#id_productcomposition_set-${newIndex}-nomenclature`),
            };
